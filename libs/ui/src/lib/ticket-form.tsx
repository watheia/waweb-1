  ...
  return formState === 'error' ? (
    ...
    <form
      ...
      onSubmit={(e) => {
        ...
        const windowTop = outerHeight / 2 + screenY - 700 / 2;
        const windowLeft = outerWidth / 2 + screenX - 600 / 2;
        ...
        new Promise<GitHubOAuthData | undefined>((resolve) => {
          ...
          window.addEventListener('message', function onMessage(msgEvent) {
            ...
            if (
              process.env['SITE_ORIGIN'] !== msgEvent.origin ||
              !msgEvent.data.type
            ) {
              return;
            }
          ...
          .then(async (data) => {
            ...
            fetch(`/tickets/${usernameFromResponse}`).catch((_) => void 0);
          })
          .catch((err) => {
        ...
        <p className={ticketFormStyles['description']}>
          {githubEnabled ? (
            ...
            <>
              ...
              <a
                href={`${config.repo}#authentication`}
                target="_blank"
